...
#    $baseform= $baseform."\#ORG=\t$orgname\n\#LINE\=\t$orgline\n\#REF=\t$topref\n";
    ...
    $baseform= "\#LOCUS= $loc_inform_txt\t\#DEF= $definit\t\#ACC= $acc\t";
    $baseform= $baseform."\#ORG= $orgname\t\#LINE\= $orgline\t\#REF= $topref";
	...
	($gnum)= &control_FeatureTable($entry,$symb,$pdir,$baseform,$info->{info}->{FEATURES}->[0],$fh_gene,$fh_pep,$$loc_inform[0]);
...
sub control_FeatureTable {
	my ($entry,$symb,$pdir,$baseform,$subinfo,$fh_gene,$fh_pep,$locus)= @_;
    ...
    if (exists $subinfo->{info}->{source}) {
        ($organism,$isolation,$clone,$db_xref)= &reform_source($subinfo->{info}->{source}->[0]);
        
        print OSO "$entry\t$baseform\t$organism\t$isolation\t$clone\t$db_xref\n";
    }
...
##    $baseform= $baseform."\#CLONE\=\t$clone\n";
	...
	if ($subitem_txt=~ /CDS|RNA/) {
		...
		foreach $subsubinfo (@all_informs) {
			$gnum++;
			$gno= $locus."_".substr($gno_ord.$gnum,length($gno_ord.$gnum)-$scale-1,$scale+1);
            # print "$gno\n";
            ...
            $glen= $subitem_txt eq "CDS" ? ($$geneforms[5]-$$geneforms[4]-2)/3 : $$geneforms[5]-$$geneforms[4]+1;
            

            print $fh_gene (join "\t",($locus,$clen,@$geneforms[0..9],"","",$glen,$$geneforms[10],$gno))."\n";
...
sub reform_source {
	my ($subsubinfo)= @_;
	my (@subsubitem,$organism,$isolation,$clone,$db_xref);

	@subsubitem= qw(organism isolation_source clone db_xref);
    ...
    $db_xref= exists $subsubinfo->{form}->{db_xref} ? $subsubinfo->{form}->{db_xref}->[0] : "";
    
  #  print "$db_xref\n";

	return ($organism,$isolation,$clone,$db_xref);
...
sub create_allgene_set {
	...
	foreach $subsubinfo (@{$subinfo->{info}->{gene}}) {
		...
		$geneinfo->{info}->{$posit}= $subsubinfo;
	}
...
sub reform_geneform {
    ...
    ###  ID: locus_tag ==> protein_id ==> gene_no
    
    $pseudo= exists $subsubinfo->{form}->{pseudo} ? 1 : 0;
    if ($pseudo == 1) {
        $fix= 2;
    } elsif ($comp == 0) {
        $fix= 3;
